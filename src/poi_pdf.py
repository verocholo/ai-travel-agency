...
def _altre_foto(tutte, escluso: str, giro: int) -> list:
    ...
    [ESTESA 2026-08-17 — task #226, richiesta di Lorenzo: «foto diverse,
    non usare sempre le solite tre ripetute».]

    Qui e' dove la ripetizione si vedeva di piu': con un itinerario piccolo
    (5-6 luoghi illustrati in tutto, come il weekend di Bologna) la STESSA
    fotografia delle Due Torri — gia' vista come apertura della SUA guida —
    tornava identica anche come "altra tappa" nelle guide vicine, piu' volte
    nello stesso fascicolo.

    Quando un luogo ha una SECONDA fotografia (`png_alt`/`credito_alt`, vedi
    `foto.raccogli_foto`) e' quella a viaggiare qui, non la principale: cosi'
    il luogo compare comunque — la promessa "ogni giornata ha le sue foto"
    non cambia — ma con uno scatto diverso da quello che il cliente ha gia'
    visto come apertura della sua guida. Dove la seconda fotografia non c'e'
    (il luogo non ne aveva una, o il tetto `foto.MAX_FOTO_SECONDARIA` era
    gia' esaurito) si usa comunque la principale: una foto ripetuta resta
    sempre meglio di nessuna foto.
    """
    if not isinstance(tutte, dict):
        return []
    elenco = []
    for chiave, scatto in tutte.items():
        if chiave == escluso or not isinstance(scatto, dict):
            continue
        if not scatto.get("png") or not scatto.get("credito"):
            continue
        alt_png = scatto.get("png_alt")
        alt_credito = scatto.get("credito_alt")
        if alt_png and alt_credito:
            elenco.append({"png": alt_png, "credito": alt_credito})
        else:
            elenco.append(scatto)
    if len(elenco) < 2:
        return []
    taglio = giro % len(elenco)
    # [PORTATE A CINQUE, 2026-08-15 — richiesta di Lorenzo: «nelle guide
    # turistiche metterei tre foto per pagina».]
    #
    # Le prime DUE vanno in cima, accanto alla fotografia del luogo: con
    # quella fanno tre sulla prima pagina. Le altre TRE fanno la fila in
    # fondo, che cade sulla seconda. Cinque e non sei perche' la fotografia
    # del luogo la scheda ce l'ha gia' sua.
    #
    # Storia di questo numero, tenuta scritta perche' e' andata avanti e
    # indietro: erano due, poi quattro per riempire la seconda pagina, poi di
    # nuovo due perche' con il corpo su due colonne sfondavano. Adesso sono
    # cinque, ma piccole — quelle di contorno vengono rimpicciolite prima di
    # entrare nel documento, quindi il file non pesa cinque volte tanto.
    return (elenco[taglio:] + elenco[:taglio])[:5]
